...
function renderDashboard(){
    ...
    rankCache = {}; const d=getDownlinesRecursive(a),e=1+d.length;
    ...
    let countDownTarget; let chartStartPrev, chartEndPrev;
    let prevM = j - 1, prevY = k; if (prevM < 0) { prevM = 11; prevY--; }
    const daysInPrevMonth = new Date(prevY, prevM + 1, 0).getDate();
    if (i === 31) { l = new Date(k, j, 31); m = new Date(k, j, 30, 23, 59, 59); n = "PERIODE 1 (BLN DEPAN)"; countDownTarget = new Date(k, j + 1, 15, 23, 59, 59); } 
    else if (i <= 15) { l = (daysInPrevMonth === 31) ? new Date(prevY, prevM, 31) : new Date(k, j, 1); m = (daysInPrevMonth === 31) ? new Date(prevY, prevM, 30, 23, 59, 59) : new Date(k, j, 0, 23, 59, 59); n = `PERIODE 1 (${getMonthName(j)})`; countDownTarget = new Date(k, j, 15, 23, 59, 59); } 
    else { l = new Date(k, j, 16); m = new Date(k, j, 15, 23, 59, 59); n = `PERIODE 2 (${getMonthName(j)})`; countDownTarget = new Date(k, j, 30, 23, 59, 59); }
    document.getElementById('currentPeriodLabel').innerText=n; startCountdown(countDownTarget); showMotivation();
    ...
    document.getElementById('prevPeriodCount').innerText = totalAccumulatedPrev; document.getElementById('targetCount').innerText = target; document.getElementById('newMemberCount').innerText = currentGrowth; document.getElementById('gapCount').innerText = gap;
...
function checkConsecutiveGrowth(uid) { return globalAchievers.filter(a => a.uid === uid).length >= 2; }
...
function getRankLevel(uid){
    ...
    const rankTiers = [{level:9,min:3501,reqLevel:8}, {level:8,min:1601,reqLevel:7}, {level:7,min:901,reqLevel:6}, {level:6,min:501,reqLevel:5}, {level:5,min:351,reqLevel:4}, {level:4,min:201,reqLevel:3}, {level:3,min:101,reqLevel:2}, {level:2,min:31,reqLevel:1}];
    let result = (direct >= 5) ? 1 : 0;
    for(const tier of rankTiers) { if (team.length >= tier.min) { const pathCount = countVipsInPaths(uid, tier.reqLevel); if (pathCount >= 2 || checkConsecutiveGrowth(uid)) { result = tier.level; break; } } }
    rankCache[uid] = result; return result;
}
...
function getHistoricalRankLevel(uid, cutoffDate) {
    ...
    let result = (direct >= 5) ? 1 : 0;
    histRankCache[cacheKey] = result; return result;
}
...
function countVipStats(a){
    ...
    a.forEach(m=>{ let c=getRankLevel(m.uid); if(c>=1&&c<=9){ b[c]++; vipLists[c].push(m); checkAndSaveHistory(m.uid, c); } });
    for(let i=1;i<=9;i++){ const el=document.getElementById(`cVIP${i}`); if(el) el.innerText=b[i]; }
}

// ============================================================
// PERBAIKAN: MODAL VIP (Urutan Baru & Prestasi Growth)
// ============================================================
window.openVipModal = function(level) {
    ...
    let startP, endP, baseCutoff;
    let prevM = m - 1, prevY = y; if (prevM < 0) { prevM = 11; prevY--; }
    const daysInPrevMonth = new Date(prevY, prevM + 1, 0).getDate();

    if (d === 31) { startP = new Date(y, m, 31); endP = new Date(y, m + 1, 15, 23, 59, 59); baseCutoff = new Date(y, m, 30, 23, 59, 59); } 
    else if (d <= 15) { startP = (daysInPrevMonth === 31) ? new Date(prevY, prevM, 31) : new Date(y, m, 1); endP = new Date(y, m, 15, 23, 59, 59); baseCutoff = (daysInPrevMonth === 31) ? new Date(prevY, prevM, 30, 23, 59, 59) : new Date(y, m, 0, 23, 59, 59); } 
    else { startP = new Date(y, m, 16); endP = new Date(y, m, 30, 23, 59, 59); baseCutoff = new Date(y, m, 15, 23, 59, 59); }

    title.innerText = `DAFTAR V.I.P ${level}`; body.innerHTML = ''; 
    
    // URUTKAN: Anggota yg baru mencapai VIP di posisi teratas
    let sorted = [...vipLists[level]].sort((a, b) => {
        ...
        const tA = hA ? new Date(hA.achieved_at).getTime() : a.joinDate.getTime();
        const tB = hB ? new Date(hB.achieved_at).getTime() : b.joinDate.getTime();
        return tB - tA;
    });
    ...
    if (sorted.length > 0) {
        sorted.forEach(mem => {
            ...
            const base = dls.filter(dl => dl.joinDate <= baseCutoff).length + 1;
            ...
            body.innerHTML += `
                <div class="v-item" style="flex-direction:column; align-items:flex-start; gap:5px; padding:15px;">
                    <div style="display:flex; justify-content:space-between; width:100%;">
                        <span class="v-n">${mem.name}</span>
                        <span class="v-u">${mem.uid}</span>
                    </div>
                    <div style="display:flex; gap:10px; font-size:10px; border-top:1px dashed #333; width:100%; padding-top:5px; margin-top:2px;">
                        <span>Target: <b style="color:#fff;">${target}</b></span>
                        <span>Capai: <b style="color:var(--gold);">${grow}</b></span>
                        <span>Sisa: <b style="color:${gap > 0 ? 'var(--danger)' : '#00ff00'};">${gap}</b></span>
                    </div>
                </div>`;
        });
...
function initNetwork(){
    ...
    const d=myTeamData.map(a=>{const b=a.joinDate;const c=`${String(b.getDate()).padStart(2,'0')}-${String(b.getMonth()+1).padStart(2,'0')}-${String(b.getFullYear()).slice(-2)}`;const rank=getRankLevel(a.uid);let starStr="";if(rank>0){starStr="⭐".repeat(rank)+" ";}return {key:a.uid,label:`${starStr}${a.uid} / ${a.name} / ${c}`,strokeColor:"#ffffff",strokeWidth:1}});
...
function openAchieverModal() {
    ...
    if (d >= 16 && d <= 30) { let lastDayPrev = new Date(y, m, 0); startP = (lastDayPrev.getDate() === 31) ? new Date(y, m - 1, 31) : new Date(y, m, 1); endP = new Date(y, m, 15, 23, 59, 59); baseCutoff = new Date(y, m - 1, 30, 23, 59, 59); plabel = `PERIODE 1 (${getMonthName(m)} ${y})`; } 
    else { let tM = m - 1, tY = y; if (d === 31) tM = m; else if (tM < 0) { tM = 11; tY--; } startP = new Date(tY, tM, 16); endP = new Date(tY, tM, 30, 23, 59, 59); baseCutoff = new Date(tY, tM, 15, 23, 59, 59); plabel = `PERIODE 2 (${getMonthName(tM)} ${tY})`; }
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            if (targetVal > 0 && rankStatus >= 1) {
                if (growCount >= targetVal) sList.push({name: mem.name, uid: mem.uid, target: targetVal, actual: growCount, rank: rankStatus});
                else fList.push({name: mem.name, uid: mem.uid, target: targetVal, actual: growCount, rank: rankStatus});
            }
        });
