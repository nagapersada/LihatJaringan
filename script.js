...
function getRankLevel(uid){
    ...
    const vip2InTeam = teamMembers.filter(m => m.uid !== uid && rankCache[m.uid] >= 2).length; 
    const vip1InTeam = teamMembers.filter(m => m.uid !== uid && rankCache[m.uid] >= 1).length;
...
// ============================================================
// BANK KALIMAT MOTIVASI & PESAN GRUP (LENGKAP)
// ============================================================
const wordBank = { 
