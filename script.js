...
// CONFIG PERINGKAT SESUAI REQUEST
const RANK_RULES = [
    ...
    { level: 1, min: 5, reqVip: 0 } // Khusus VIP 1: Logic direct downline ada di function
];
...
async function loadData() {
    try {
        ...
        // [OTOMATIS] BERSIHKAN DATA YG TIDAK VALID SAAT LOAD
...
// FUNGSI PEMBERSIH DATA HISTORY (VIP 1,2,3 yg tidak sah dihapus)
async function cleanInvalidVipHistory() {
    const invalidIds = [];
    
    globalHistory.forEach(hist => {
        // Cek apakah member ini masih layak memegang rank tersebut saat ini
        const currentRealRank = getRankLevel(hist.uid);
        if (hist.vip_level > currentRealRank) {
            // console.warn(`Data tidak valid ditemukan: ${hist.uid} level ${hist.vip_level}. Real: ${currentRealRank}`);
            invalidIds.push(hist.id);
        }
    });

    if (invalidIds.length > 0) {
        // Hapus dari memori lokal agar tampilan benar
        globalHistory = globalHistory.filter(h => getRankLevel(h.uid) >= h.vip_level);
        
        // Catatan: Penghapusan database ditahan agar tidak membebani server client-side, 
        // tapi secara visual di aplikasi sudah bersih karena filter di atas.
    }
}
...
function getRankLevel(uid) {
    ...
    const totalTeam = team.length + 1; // +1 Diri sendiri (sesuai request)
    const directs = getDirectDownlines(uid);

    let currentLevel = 0;

    // RULE KHUSUS VIP 1: Downline Langsung 5 Anggota
    if (directs.length >= 5) {
        currentLevel = 1;
    }
    ...
    for (const rule of RANK_RULES) {
        if (rule.level === 1) continue; // Skip VIP 1 krn logic khusus diatas

        // Syarat 1: Jumlah Total
        if (totalTeam >= rule.min) {
            
            // Syarat 2: Structure (2 Kaki Berbeda)
            const hasStructure = checkStructureCondition(uid, rule.reqVip);
            
            // Syarat 3: Growth 50% 2 Periode Berturut-turut
            const hasGrowth = checkGrowthCondition(uid, team, rule.min);

            if (hasStructure || hasGrowth) {
                // Jika memenuhi syarat Struktur ATAU Growth, update level
...
// Cek Struktur: 2 Kaki Berbeda mengandung VIP tertentu
...
// Cek Growth: 50% berturut-turut pada bulan berjalan dan bulan berikutnya
function checkGrowthCondition(uid, teamMembers, minMembersReq) {
    ...
    // Member yang men-trigger jumlah terpenuhi
    ...
    if (new Date() < p2Dates.end) return false; // P2 belum selesai
...
// ============================================================
// MANAJEMEN TANGGAL & PERIODE (DIPERBAIKI)
// ============================================================
...
function getPeriodFromDate(date) {
    ...
    if (d >= 16) {
        ...
        // Start bisa jadi tgl 30 atau 31 tergantung bulan lalu
...
function calculatePeriodGrowth(uid, startDate, endDate) {
    ...
    const baseCount = 1 + downlinesBefore; // +1 Anda
...
function renderDashboard(){
    ...
    // Label Dashboard mengacu pada Tanggal Akhir periode agar konsisten
    const isP1 = currP.end.getDate() <= 15;
    document.getElementById('currentPeriodLabel').innerText = `PERIODE ${isP1?1:2} (${getMonthName(currP.end.getMonth())})`;
    
    startCountdown(currP.end);
    showMotivation();

    // Hitung Statistik Periode Berjalan
    ...
    if (nextRule) {
        if (nextLvl === 1) {
            ...
            if (gapMem > 0) {
                ...
                goalEl.innerText = `Syarat: 2 Kaki @VIP${nextRule.reqVip} ATAU Growth 50%`;
                gapEl.innerText = "0";
            }
        }
    } else {
...
// ============================================================
// TOMBOL PERAIH (PERBAIKAN LOGIC TANGGAL)
// ============================================================
...
window.openAchieverModal = function() {
    ...
    // [FIX] Menggunakan Tanggal AKHIR periode untuk menentukan Nama Bulan/Periode
    // Karena Start Date bisa tgl 31 bulan sebelumnya (sehingga terbaca salah bulan).
    // Tgl akhir (15 atau 30/31) selalu berada di bulan yang benar.
    const isPrvP1 = prevP.end.getDate() <= 15;
    const pName = isPrvP1 ?
        `PERIODE 1 (${getMonthName(prevP.end.getMonth())})` :
        `PERIODE 2 (${getMonthName(prevP.end.getMonth())})`;
...
function renderAchieverList(successList, failureList, body, btnDl) {
    successList.sort((a,b) => { if (b.rank !== a.rank) return b.rank - a.rank; return b.actual - a.actual; });
    failureList.sort((a,b) => { if (b.rank !== a.rank) return b.rank - a.rank; return a.gap - b.gap; });

    let html = ''; const myUid = sessionStorage.getItem('userUid'); btnDl.style.display = 'block';
    ...
    if (successList.length > 0) {
        ...
        successList.forEach((a,i) => { 
            ...
            html += `<div class="achiever-item" style="border-left:3px solid #00ff00; ${isMe?'background:#1a2a1a;':''}"><div class="achiever-top"><span class="v-n">${i+1}. ${a.name} ${isMe?'(ANDA)':''} <small style="color:var(--gold)">(VIP ${a.rank})</small></span><span class="v-u">${a.uid}</span></div><div class="achiever-stats"><span>Target: <b class="val-target">${a.target}</b></span><span>Capai: <b class="val-actual" style="color:#00ff00;">${a.actual}</b></span><span>Sisa: <b style="color:#00ff00;">0</b></span></div></div>`; 
    ...
    if (failureList.length > 0) {
        ...
        failureList.forEach((a,i) => { 
            const isMe = (a.uid === myUid);
            html += `<div class="achiever-item" style="border-left:3px solid #ff4444; ${isMe?'background:#2a1a1a;':''}"><div class="achiever-top"><span class="v-n">${i+1}. ${a.name} ${isMe?'(ANDA)':''} <small style="color:#888">(VIP ${a.rank})</small></span><span class="v-u">${a.uid}</span></div><div class="achiever-stats"><span>Target: <b class="val-target">${a.target}</b></span><span>Capai: <b style="color:#ff4444;">${a.actual}</b></span><span>Sisa: <b style="color:#ff4444;">${a.gap}</b></span></div></div>`; 
...
function renderTable(){
    ...
    let e=''; d.forEach((a,b)=>{const c=a.joinDate,d=`${String(c.getDate()).padStart(2,'0')}/${String(c.getMonth()+1).padStart(2,'0')}/${c.getFullYear()}`,f=a.upline?a.upline:'-';e+=`<tr><td class="col-no">${b+1}</td><td class="col-name">${a.name}</td><td class="col-uid">${a.uid}</td><td class="col-ref">${f}</td><td class="col-date">${d}</td></tr>`}); a.innerHTML=e;
}
...
function initNetwork(){
    ...
    const d=myTeamData.map(a=>{const b=a.joinDate,c=`${String(b.getDate()).padStart(2,'0')}-${String(b.getMonth()+1).padStart(2,'0')}-${String(b.getFullYear()).slice(-2)}`,d=getRankLevel(a.uid),e=d>=1;let f="";if(d>0)f="⭐".repeat(d)+" ";return{key:a.uid,label:`${f}${a.uid} / ${a.name} / ${c}`,strokeColor:e?"#ffd700":"#ffffff",strokeWidth:e?2:1}});
...
let intervalId; function startCountdown(t){if(intervalId)clearInterval(intervalId);function u(){const n=new Date().getTime(),d=t.getTime()-n;if(d<0){document.getElementById("cdDays").textContent="00";document.getElementById("cdHours").textContent="00";document.getElementById("cdMin").textContent="00";document.getElementById("cdSec").textContent="00";return}const a=Math.floor(d/(1e3*60*60*24)),b=Math.floor(d%(1e3*60*60*24)/(1e3*60*60)),c=Math.floor(d%(1e3*60*60)/(1e3*60)),e=Math.floor(d%(1e3*60)/1e3);document.getElementById("cdDays").textContent=a<10?"0"+a:a;document.getElementById("cdHours").textContent=b<10?"0"+b:b;document.getElementById("cdMin").textContent=c<10?"0"+c:c;document.getElementById("cdSec").textContent=e<10?"0"+e:e}u(),intervalId=setInterval(u,1e3)}
...
function generateDynamicQuote(){const r=a=>a[Math.floor(Math.random()*a.length)];return`"${r(wordBank.openings)} ${r(wordBank.conditions)} ${r(wordBank.actions)} ${r(wordBank.closings)}"`}
function showMotivation(){if(!sessionStorage.getItem('isLoggedIn'))return;const a=document.getElementById('motivationModal'),b=document.getElementById('dailyMotivation');a&&b&&(b.innerText=generateDynamicQuote(),a.style.display='flex')}
window.closeMotivation=function(){const a=document.getElementById('motivationModal');a&&(a.style.opacity='0',setTimeout(()=>{a.style.display='none',a.style.opacity='1'},300))}
...
window.generateShareText=function(){const a=document.getElementById('shareText'),b=a=>a[Math.floor(Math.random()*a.length)],c=['Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'],d=['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'],e=new Date,f=`${c[e.getDay()]}, ${e.getDate()} ${d[e.getMonth()]}`;a.value=`${b(groupWords.headers)} \nDVTEAM NP 🔥\n\n📅 ${f}\n👥 Target 50%: *${globalTarget} Anggota*\n\n"${b(groupWords.intros)}"\n\n🚀 ${b(groupWords.alerts)} = sisa ${currentGap} anggota lagi..!! 🏆\n${b(groupWords.closings)}\n#DVTeamNP #SatuVisi #SatuMisi #SatuKomando`};
window.copyShareText=function(){const a=document.getElementById('shareText');a.select(),a.setSelectionRange(0,99999),navigator.clipboard.writeText(a.value).then(()=>{alert("✅ Teks berhasil disalin! Silakan tempel di Grup WhatsApp.")})};
