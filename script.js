...
function calculateSlideData(myUid) {
    ...
    // 2. DATA PEJUANG ROADMAP90 (URUT DARI YANG TERBARU TEMBUS & SEMUA GOLD)
    const rmContainer = document.getElementById('roadmapList');
    const rmCountEl = document.getElementById('rmCount');
    let rmHTML = '';

    // A. Kumpulkan Kandidat & Tentukan Tanggal Kualifikasi
    let qualifiers = [];

    globalData.forEach(member => {
        const directs = getDirectDownlines(member.uid);

        // Cari direct yang VIP (Rank >= 1)
        const vipDirects = directs.filter(d => getRankLevel(d.uid) >= 1);

        if (vipDirects.length >= 5) {
            // Tentukan kapan member ini "Tembus" (Saat downline ke-5 naik VIP)
            // Kita ambil tanggal achieve dari setiap direct VIP
            const achieveTimes = vipDirects.map(d => getEstAchieveDate(d, 1).getTime());

            // Urutkan tanggal achieve dari yang terlama ke terbaru
            achieveTimes.sort((a,b) => a - b);

            // Waktu tembus adalah waktu downline ke-5 mencapai VIP
    ...
    // B. Urutkan Kandidat: Waktu Tembus Terbaru di Atas (Descending)
