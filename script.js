...
let currentGap = 0, globalTarget = 0;
    ...
    if(!list.length) body.innerHTML = '<div class="v-empty">Belum ada data.</div>';
        ...
        list.forEach(mMember => {
            ...
            body.innerHTML += `<div class="v-item" style="flex-direction:column; align-items:flex-start;">
                ...
                <div style="font-size:10px; color:#888; margin-top:4px;">Target: ${target} - Capai: <span class="text-gold">${grow}</span> - Sisa: <span style="color:${sisa > 0 ? '#ff4d4d' : '#00ff00'};">${sisa}</span></div>
            </div>`;
        });
    }
...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            if (target > 0 && rank >= 1) {
                if (grow >= target) sList.push({name: mem.name, uid: mem.uid, target, actual: grow, rank});
                else fList.push({name: mem.name, uid: mem.uid, target, actual: grow, rank});
            }
        });
...
// ============================================================
const wordBank = { 
    openings: ["🔥 SAYA SIAP BERTEMPUR!", "⚠️ PERINGATAN KERAS!", "🚀 JANGAN LENGAH!", "🦁 BANGKITLAH PEMENANG!"], 
    conditions: ["Hari ini musuhmu sedang bekerja keras,", "Target tidak akan menunggu kamu siap,", "Dunia tidak peduli dengan alasanmu,"], 
    actions: ["maka HANCURKAN rasa malasmu sekarang!", "segera RAPATKAN BARISAN!", "tunjukkan siapa RAJA di sini!"], 
    closings: ["PASTI BISA!", "GASPOLL!", "TUNTASKAN!"] 
};
...
function downloadNetworkImage(){ alert("Fungsi ini membutuhkan GoJS License untuk ekspor gambar."); }
