...
function renderDashboard(){
    ...
    // UPDATE: INITIALIZE SLIDER CAROUSEL
...
function initCarousel() {
    ...
    slideInterval = setInterval(() => {
        idx = (idx + 1) % 3; // Loop 0, 1, 2
        track.style.transform = `translateX(-${idx * 33.33}%)`; // Geser per 1/3 lebar (karena width track 300%)
...
function calculateSlideData(myUid) {
    ...
    // Mulai dari periode sebelumnya (bukan periode berjalan)
    let checkDate = new Date(currentP.start.getTime() - 10000); 
    
    let periodsFound = 0;
    
    // Loop max 6 periode ke belakang
    while (periodsFound < 6) {
        const p = getPeriodFromDate(checkDate);
        
        // Cek apakah user sudah join di periode ini
        // Menggunakan globalData (pastikan sudah diload)
    ...
    // 2. DATA PEJUANG ROADMAP90 (Punya 5 Direct VIP 1)
    const rmContainer = document.getElementById('roadmapList');
    const rmCountEl = document.getElementById('rmCount');
    let rmHTML = '';
    let rmTotal = 0;

    // Filter tim saya (myTeamData sudah di-load di renderDashboard)
    // Urutkan berdasarkan total downline terbanyak
    const sortedTeam = [...myTeamData].sort((a,b) => {
    ...
    sortedTeam.forEach(member => {
        // Disini kita filter anggota tim (downline).
