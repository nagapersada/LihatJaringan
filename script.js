...
function renderDashboard(){
    ...
    if (i === 31) {
        l = new Date(k, j, 31); 
        m = new Date(k, j, 30, 23, 59, 59); // Cut-off Total Periode Lalu
...
function generateShareText() { const txtArea = document.getElementById('shareText'); const r = (arr) => arr[Math.floor(Math.random() * arr.length)]; const now = new Date(); const dateStr = `${now.getDate()}/${now.getMonth()+1}/${now.getFullYear()}`; const finalTxt = `🔥 UPDATE MEMBARA \nDVTEAM NP 🔥\n\n📅 ${dateStr}\n👥 Target 50%: *${globalTarget} Anggota*\n\n"Keberhasilan tim bukan dinilai dari seberapa cepat kita berlari."\n\n🚀 Tetap fokus pada pertumbuhan 50% = sisa ${currentGap} anggota lagi..!! 🏆\n💪🏻 Gow .. Gaaaaass sampai tembus..!!\n#DVTeamNP #SatuVisi`; txtArea.value = finalTxt; }
function copyShareText() { const txtArea = document.getElementById('shareText'); txtArea.select(); navigator.clipboard.writeText(txtArea.value).then(() => { alert("✅ Teks berhasil disalin!"); }); }
