...
function renderDashboard(){
    ...
    const base = myTeamData.filter(mem => mem.joinDate <= m).length;
    const grow = myTeamData.filter(mem => mem.joinDate >= l).length;
...
// ============================================================
// MODAL VIP & PERAIH (KEMBALI KE ASAL)
// ============================================================
...
function openAchieverModal() {
    ...
    modal.style.display = 'flex'; body.innerHTML = '<div class="v-empty">Menghitung data peraih...</div>'; btnDl.style.display = 'none';
    const h=new Date(),i=h.getDate(),j=h.getMonth(),k=h.getFullYear();
    let l,m,plabel;
    let prevM = j - 1, prevY = k; if (prevM < 0) { prevM = 11; prevY--; }
    const daysInPrevMonth = new Date(prevY, prevM + 1, 0).getDate();
    ...
    title.innerText = `REPORT PERAIH - ${plabel}`; achieverTxtContent = `REPORT PERAIH ${plabel}\n\n`;
    setTimeout(() => {
        let sList = [], fList = [];
        myTeamData.forEach(mem => {
            ...
            if (getRankLevel(mem.uid) >= 1 && targetVal > 0) {
                if (growCount >= targetVal) sList.push({name: mem.name, uid: mem.uid, target: targetVal, actual: growCount});
                else fList.push({name: mem.name, uid: mem.uid, target: targetVal, actual: growCount});
            }
        });
...
function renderAchieverList(s, f, body, btn) {
    ...
    if (f.length > 0) {
        html += `<div style="padding:10px; color:#ff4d4d; font-weight:bold; margin-top:10px;">⚠️ BELUM TERCAPAI</div>`;
...
// ============================================================
// FUNGSI JARINGAN (DIAGRAM GOJS)
// ============================================================
function initNetwork(){
    const a=sessionStorage.getItem('userUid'),b=go.GraphObject.make,
    c=b(go.Diagram,"networkDiagram",{
        padding:new go.Margin(150),
        layout:b(go.TreeLayout,{angle:90,layerSpacing:80}),
        "undoManager.isEnabled":true
    });
    c.nodeTemplate=b(go.Node,"Vertical",
        b(go.Panel,"Auto",
            b(go.Shape,"RoundedRectangle",{fill:"#111",stroke:"#D4AF37",strokeWidth:2}),
            b(go.TextBlock,{margin:8,stroke:"#fff",font:"bold 12px sans-serif"},new go.Binding("text","name"))
        ),
        b(go.TextBlock,{stroke:"#D4AF37",font:"10px sans-serif"},new go.Binding("text","uid"))
    );
    c.linkTemplate=b(go.Link,{routing:go.Link.Orthogonal,corner:5},b(go.Shape,{stroke:"#444",strokeWidth:1}));
    const nodes = myTeamData.map(m=>({key:m.uid,name:m.name,uid:m.uid}));
    const links = myTeamData.filter(m=>m.upline).map(m=>({from:m.upline,to:m.uid}));
    c.model = new go.GraphLinksModel(nodes,links);
}

function downloadNetworkImage(){
    const diagram = go.Diagram.fromDiv("networkDiagram");
    if(diagram){
        const img = diagram.makeImage({scale:2,background:"#000"});
        const a = document.createElement("a"); a.href=img.src; a.download="jaringan_dvteam.png"; a.click();
    }
}

// Sisa fungsi penunjang (Motivation, Share, Countdown, Logout)
...
function startCountdown(target){
    ...
    window.iv = setInterval(()=>{
        const diff = target - new Date(); if(diff <= 0) return clearInterval(window.iv);
...
function showMotivation() { const modal = document.getElementById('motivationModal'); if(modal) modal.style.display = 'flex'; }
...
function downloadAchieverData() { const blob=new Blob([achieverTxtContent],{type:'text/plain'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='report_peraih.txt'; a.click(); }
