...
function renderDashboard(){
    ...
    // UPDATE: INITIALIZE SLIDER CAROUSEL (SWIPE + INFINITE)
...
function initCarousel() {
    ...
    // 1. CLONING UNTUK INFINITE LOOP (Jika belum di-clone)
    if (slides.length === 3) {
    ...
    // Karena ada 4 slide (3 asli + 1 clone), lebar track harus 400%
    // Dan lebar setiap slide 25% (diatur di CSS sudah benar)
    ...
    // --- INFINITE LOOP LOGIC (TRANSITION END) ---
    track.addEventListener('transitionend', () => {
        if (slideIndex === 3) { // Jika sampai di Clone
            track.style.transition = "none"; // Matikan animasi
            slideIndex = 0; // Lompat balik ke index 0 (asli) secara instan
            track.style.transform = `translateX(0%)`;
            setTimeout(() => {
                track.style.transition = "transform 0.5s ease-in-out"; // Hidupkan lagi animasi
            });
        }
...
function moveSlide(direction) {
    ...
    // Geser Track
    // Karena lebar slide 25% (100/4), geser berdasarkan index * 25
    track.style.transform = `translateX(-${slideIndex * 25}%)`;
    
    // Update Dots (Handle kasus clone)
    let dotIndex = slideIndex;
    if (dotIndex === 3) dotIndex = 0; // Jika di clone, nyalakan dot pertama
...
function touchStart(index) {
    return function(event) {
        ...
        isDragging = true;
        stopAutoSlide(); // Stop auto saat disentuh
    }
}

function touchMove(event) {
    if (isDragging) {
        // Bisa ditambahkan logika visual dragging realtime disini jika mau
    }
}
...
function touchEnd(event) {
    ...
    if (movedBy < -50) {
        moveSlide(1); // Swipe Kiri -> Next
    } else if (movedBy > 50) {
        if(slideIndex > 0) moveSlide(-1); // Swipe Kanan -> Prev (Kecuali di awal)
    }
    
    startAutoSlide(); // Resume auto
}

// Helper untuk touch listener
...
function calculateSlideData(myUid) {
    ...
    while (periodsFound < 6) {
        ...
        const pName = getMonthName(p.end.getMonth()) + (p.end.getDate() > 15 ? ' (2)' : ' (1)');
        
        histHTML += `
        <div class="hist-row">
            <span>${pName} ${p.end.getFullYear()}</span>
            <span class="${isSuccess ? 'hist-success' : 'hist-fail'}">
                ${isSuccess ? '🏆 TERCAPAI' : '❌ GAGAL'} (${stats.actual}/${stats.target})
            </span>
        </div>`;
    ...
    if(histHTML === '') histHTML = '<div style="text-align:center; padding-top:20px;">Belum ada riwayat.</div>';
    histContainer.innerHTML = histHTML;


    // 2. DATA PEJUANG ROADMAP90 (GLOBAL - SELURUH ANGGOTA)
    const rmContainer = document.getElementById('roadmapList');
    const rmCountEl = document.getElementById('rmCount');
    let rmHTML = '';
    let rmTotal = 0;

    // GANTI: Menggunakan globalData (Seluruh Anggota Naga Persada)
    // Filter dan urutkan berdasarkan jumlah downline langsung
    const sortedGlobal = [...globalData].sort((a,b) => {
        const da = getDirectDownlines(a.uid).length;
        const db = getDirectDownlines(b.uid).length;
        return db - da;
    });

    sortedGlobal.forEach(member => {
        // Cek Downline Langsung dia
        const directs = getDirectDownlines(member.uid);
        
        // Syarat: Memiliki 5 downline langsung berperingkat MINIMAL VIP 1
        let qualifiedDirects = 0;
        directs.forEach(d => {
            if (getRankLevel(d.uid) >= 1) qualifiedDirects++;
        });

        if (qualifiedDirects >= 5) {
            rmTotal++;
            rmHTML += `
            <div class="rm-row">
                <span>${member.name}</span>
                <span style="color:var(--gold)">${qualifiedDirects} Jalur VIP</span>
            </div>`;
        }
    });

    if (rmTotal === 0) rmHTML = '<div style="text-align:center; padding-top:20px;">Belum ada Qualifier.</div>';
    
    rmContainer.innerHTML = rmHTML;
    rmCountEl.innerText = rmTotal;
}
