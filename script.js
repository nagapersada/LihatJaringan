...
async function loadData() {
    ...
    } catch (a) { console.error("Load Error:", a); alert("Gagal memuat data."); }
}
...
function renderDashboard(){
    ...
    if (i === 31) {
        ...
        n = "PERIODE 1 (JAN 2026)"; startCountdown(new Date(k, j + 1, 15, 23, 59, 59));
    } else if (i <= 15) {
        ...
        n = `PERIODE 1 (${getMonthName(j)} ${k})`; startCountdown(new Date(k, j, 15, 23, 59, 59));
    } else {
        l = new Date(k, j, 16); m = new Date(k, j, 15, 23, 59, 59);
        n = `PERIODE 2 (${getMonthName(j)} ${k})`; startCountdown(new Date(k, j, 30, 23, 59, 59));
    }
...
function renderChart(p, c){
    const ctx = document.getElementById('growthChart').getContext('2d');
    if(window.myChart) window.myChart.destroy();
    window.myChart = new Chart(ctx, { type: 'bar', data: { labels: ['LALU', 'SAAT INI'], datasets: [{ data: [p, c], backgroundColor: ['#333', '#D4AF37'] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } });
}

// ============================================================
// LOGIKA PERINGKAT (STRATEGI JALUR & PERTUMBUHAN 50%)
// ============================================================
...
function initNetwork(){
    const b=go.GraphObject.make, c=b(go.Diagram,"networkDiagram",{layout:b(go.TreeLayout,{angle:0,layerSpacing:60})});
    c.nodeTemplate=b(go.Node,"Auto",b(go.Shape,"RoundedRectangle",{fill:"#000",stroke:"#D4AF37"}),b(go.TextBlock,{margin:8,stroke:"#fff"},new go.Binding("text","label")));
...
function openAchieverModal() {
    ...
    const h=new Date(), i=h.getDate(), j=h.getMonth(), k=h.getFullYear();
    let l, m, plabel;
    if (i >= 16) { l = new Date(k, j, 1); m = new Date(k, j, 15, 23, 59, 59); plabel = "PERIODE 1"; }
...
function renderAchieverList(s, f, body, btn) {
    ...
    if (s.length > 0) { html += `<div style="padding:10px; color:#00ff00;">🏆 LOLOS TARGET</div>`; s.forEach(a => html += `<div class="achiever-item"><b>${a.name}</b><br>Target: ${a.target} | Capai: ${a.actual}</div>`); }
    if (f.length > 0) { html += `<div style="padding:10px; color:#ff4444;">⚠️ BELUM LOLOS</div>`; f.forEach(a => html += `<div class="achiever-item"><b>${a.name}</b><br>Target: ${a.target} | Capai: ${a.actual}</div>`); }
    body.innerHTML = html || 'Tidak ada data.';
}

const wordBank = { openings: ["🔥 SAYA SIAP BERTEMPUR!", "⚠️ PERINGATAN KERAS!"], conditions: ["Hari ini musuhmu sedang bekerja keras,"], actions: ["maka HANCURKAN rasa malasmu sekarang!"], closings: ["PASTI BISA!", "GASPOLL!"] };
...
function downloadAchieverData() { const b = new Blob([achieverTxtContent], {type:'text/plain'}); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'laporan.txt'; a.click(); }
