    ...
    if(!list.length) body.innerHTML = '<div class="v-empty">Belum ada data.</div>';
        ...
        list.forEach(mMember => {
            ...
            body.innerHTML += `<div class="v-item" style="flex-direction:column; align-items:flex-start;">
                ...
                <div style="font-size:10px; color:#888; margin-top:4px;">Target: ${target} - Capai: <span class="text-gold">${grow}</span> - Sisa: <span class="${sisa > 0 ? 'text-red' : ''}">${sisa}</span></div>
            </div>`;
        });
    }
...
// Kembalikan sisa fungsi aslinya: openAchieverModal, shareText, dll.
