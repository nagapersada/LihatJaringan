...
function getRankLevel(uid){
    ...
    let result = (directDownlinesCount >= 5) ? 1 : 0;
    for(const t of rankTiers) {
        if (totalMembers >= t.min) {
            ...
            if (paths >= 2 || growthSuccess) { result = t.level; break; }
        }
    }
...
function getHistoricalRankLevel(uid, cutoffDate) {
    const teamMembers = [globalData.find(m => m.uid === uid), ...getDownlinesRecursive(uid)].filter(m => m.joinDate <= cutoffDate);
    const directDownlinesCount = globalData.filter(b => b.upline === uid && b.joinDate <= cutoffDate).length;
...
window.openVipModal = function(level) {
    ...
    let l, cutoff;
    if (i <= 15) { l = new Date(y, m, 1); cutoff = new Date(y, m, 0, 23, 59, 59); } 
    else { l = new Date(y, m, 16); cutoff = new Date(y, m, 15, 23, 59, 59); }
    ...
    if (sorted.length > 0) {
        sorted.forEach(mem => {
            ...
            const target = Math.ceil((dls.filter(d => d.joinDate <= cutoff).length + 1) / 2);
            ...
            body.innerHTML += `<div class="v-item" style="flex-direction:column; align-items:flex-start;">
                <div style="display:flex; justify-content:space-between; width:100%;"><span class="v-n">${mem.name}</span><span class="v-u">${mem.uid}</span></div>
