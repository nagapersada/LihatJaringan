...
// ============================================================
// PERBAIKAN LOGIKA PERIODE LAPORAN (SINKRON INSTRUKSI)
// ============================================================
function openAchieverModal() {
    ...
    if (d >= 16 && d <= 30) { 
        // JIKA TGL 16-30: LIHAT HASIL PERIODE 1 (31 s/d 15)
        let prevM = m, prevY = y;
        let lastDayPrev = new Date(y, m, 0); 
        if (lastDayPrev.getDate() === 31) {
            startP = new Date(y, m - 1, 31);
        } else {
        ...
        // JIKA TGL 31 s/d 15: LIHAT HASIL PERIODE 2 BULAN LALU (16 s/d 30)
        let targetM = m - 1; let targetY = y; 
        if (d === 31) { targetM = m; } // Jika tgl 31 hari ini, maka periode 2 yg baru lewat adalah bulan ini
        else if (targetM < 0) { targetM = 11; targetY--; }
        
        startP = new Date(targetY, targetM, 16); 
        endP = new Date(targetY, targetM, 30, 23, 59, 59); 
        plabel = `PERIODE 2 (${getMonthName(targetM)} ${targetY})`; 
    }
