...
function renderDashboard(){
    ...
    const base = g.filter(a => a.joinDate <= m).length;
    const grow = g.filter(a => a.joinDate >= l).length;
...
function getRankLevel(uid){
    ...
    for(const t of rankTiers) {
        if (team.length >= t.min) {
            const paths = globalData.filter(m=>m.upline===uid).filter(d => [d, ...getDownlinesRecursive(d.uid)].some(m => getRankLevel(m.uid) >= t.req)).length;
...
function countVipStats(a){
    ...
    a.forEach(m=>{ let c=getRankLevel(m.uid); if(c>=1&&c<=9){ b[c]++; vipLists[c].push(m); } });
    for(let i=1;i<=9;i++){ const el=document.getElementById(`cVIP${i}`); if(el) el.innerText=b[i]; }
}

// KEMBALIKAN FUNGSI MODAL VIP UTUH
window.openVipModal = function(level) {
    ...
    let sorted = [...vipLists[level]].sort((a, b) => b.joinDate - a.joinDate);
    if (sorted.length > 0) {
        sorted.forEach(m => { body.innerHTML += `<div class="v-item"><span class="v-n">${m.name}</span><span class="v-u">${m.uid}</span></div>`; });
...
// KEMBALIKAN FUNGSI DIAGRAM JARINGAN UTUH
function initNetwork(){
    const a=sessionStorage.getItem('userUid'),b=go.GraphObject.make,
    c=b(go.Diagram,"networkDiagram",{padding:new go.Margin(150),layout:b(go.TreeLayout,{angle:0,layerSpacing:60})});
    c.nodeTemplate=b(go.Node,"Horizontal",
        b(go.Panel,"Auto",
            b(go.Shape,"RoundedRectangle",{fill:"#000",stroke:"#D4AF37"}),
            b(go.TextBlock,{margin:5,stroke:"#fff"},new go.Binding("text","label"))
        )
    );
...
// KEMBALIKAN FUNGSI PERAIH NAVIGASI BAWAH UTUH
function openAchieverModal() {
    ...
    if (i >= 16 && i <= 30) {
        ...
        l = new Date(prevY, prevM, 16);
        m = new Date(prevY, prevM, 30, 23, 59, 59);
        baseCut = new Date(prevY, prevM, 15, 23, 59, 59);
        plabel = "PERIODE 2";
    }

    title.innerText = `LAPORAN ${plabel}`;
    setTimeout(() => {
        let sList = [], fList = [];
...
function renderAchieverList(s, f, body, btn) {
    ...
    if (s.length > 0) {
        html += `<div style="padding:10px; color:#00ff00;">🏆 LOLOS TARGET</div>`;
        s.forEach(a => html += `<div class="achiever-item"><b>${a.name}</b><br>Target: ${a.target} | Capai: ${a.actual}</div>`);
    }
    if (f.length > 0) {
        html += `<div style="padding:10px; color:#ff4444;">⚠️ BELUM LOLOS</div>`;
...
function startCountdown(t){if(window.iv)clearInterval(window.iv);window.iv=setInterval(()=>{const d=t-new Date();if(d<0)return;document.getElementById('cdDays').innerText=Math.floor(d/864e5)},1000)}
