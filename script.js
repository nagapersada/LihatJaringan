...
function getEstAchieveDate(member, rank) {
    // 1. Cek History Database (Prioritas)
    const hist = globalHistory.find(h => h.uid === member.uid && h.vip_level === rank);
    if (hist) return new Date(hist.achieved_at);

    // 2. Jika tidak ada history, cek tanggal join downline penentu (Untuk VIP 1)
    if (rank === 1) {
        const directs = getDirectDownlines(member.uid).sort((a,b) => a.joinDate - b.joinDate);
        if (directs.length >= 5) {
            // Tanggal qualify adalah tanggal join downline ke-5
            return directs[4].joinDate; 
        }
    }
    
    // Fallback: Tanggal join member itu sendiri (jika tidak bisa dilacak)
...
function countVipStats(){
    ...
    myTeamData.forEach(m => {
        ...
        if (r >= 1 && r <= 9) {
            ...
            // GUNAKAN EST ACHIEVEMENT DATE AGAR AKURAT
...
window.openVipModal = function(level) {
    ...
    const list = vipLists[level].sort((a,b) => {
        // SORTING BERDASARKAN TANGGAL TEMBUS, BUKAN JOIN
...
function touchMove(event) { if (isDragging) { /* Visual feedback jika diperlukan */ } }
...
function calculateSlideData(myUid) {
    // 1. DATA HISTORY GROWTH 50% (DESIGN ELEGANT)
    ...
    // 2. DATA PEJUANG ROADMAP90 (GLOBAL & LEADERBOARD STYLE)
    ...
    sortedGlobal.forEach((member, index) => {
        const directs = getDirectDownlines(member.uid);
        let qualifiedDirects = 0;
        directs.forEach(d => { if (getRankLevel(d.uid) >= 1) qualifiedDirects++; });

        if (qualifiedDirects >= 5) {
            rmTotal++;
            const isTop3 = (rmTotal <= 3) ? 'top3' : '';
            rmHTML += `
            <div class="rm-card">
                <div class="rm-left">
                    <div class="rm-rank ${isTop3}">${rmTotal}</div>
                    <span class="rm-name">${member.name}</span>
                </div>
                <span class="rm-badge">${qualifiedDirects} JALUR</span>
            </div>`;
        }
    });

    if (rmTotal === 0) rmHTML = '<div style="text-align:center; padding-top:20px; color:#666;">Belum ada Qualifier.</div>';
