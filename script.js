...
// ============================================================
// PERBAIKAN LOGIKA PERAIH (31-15 & 16-30)
// ============================================================
...
function showMotivation() { if (!sessionStorage.getItem('isLoggedIn')) return; const modal = document.getElementById('motivationModal'); if (modal) { modal.style.display = 'flex'; } }
...
window.generateShareText = function() { const txtArea = document.getElementById('shareText'); const r = (arr) => arr[Math.floor(Math.random() * arr.length)]; const now = new Date(); const dateStr = `${now.getDate()}/${now.getMonth()+1}/${now.getFullYear()}`; const finalTxt = `${r(groupWords.headers)} \\nDVTEAM NP 🔥\\n\\n📅 ${dateStr}\\n👥 Target 50%: *${globalTarget} Anggota*\\n\\n\"${r(groupWords.intros)}\"\\n\\🚀 ${r(groupWords.alerts)} = sisa ${currentGap} anggota lagi..!! 🏆\\n${r(groupWords.closings)}\\n#DVTeamNP #SatuVisi`; txtArea.value = finalTxt; }
window.copyShareText = function() { const txtArea = document.getElementById('shareText'); txtArea.select(); navigator.clipboard.writeText(txtArea.value).then(() => { alert(\"✅ Teks berhasil disalin!\"); }); }
