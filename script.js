...
// LOGIKA KENAIKAN PERINGKAT BARU
const RANK_RULES = {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if(!isLogin && !window.location.pathname.includes('index.html')) { window.location.href='index.html'; return; }
    await loadData();
...
function renderDashboard() {
    ...
    if(d <= 15 || d === 31) {
        ...
        label = `PERIODE 1 (${getMonthName(now.getMonth())})`;
    } else {
        start = new Date(now.getFullYear(), now.getMonth(), 16);
        end = new Date(now.getFullYear(), now.getMonth(), 30, 23, 59, 59);
        label = `PERIODE 2 (${getMonthName(now.getMonth())})`;
    }
...
function calculateNextRank(uid, currentSize) {
    ...
    const curRank = RANK_RULES[curLevel] || { name: "MEMBER", level: 0 };
    ...
    if (curLevel === 9) sg = "Top Level";
        ...
        if (vips < next.reqVipCount) sg = `Tambahan ${next.reqVipCount - vips} @VIP ${next.reqLevel} di jalur berbeda`;
...
function openVipModal(level) {
    ...
    // Perbaikan Periode untuk Modal Detail
    const now = new Date();
    const d = now.getDate();
    let start;
    ...
    vips.forEach(m => {
        ...
        // Logika Target per anggota di daftar
        ...
        body.innerHTML += `<div class="v-item ${isNew?'new-name-alert':''}">
            <div style="display:flex; flex-direction:column; width:100%;">
                <div style="display:flex; justify-content:space-between;">
                    <span class="v-n">${m.name} ${isNew?'<span class="badge-new">🔥 BARU</span>':''}</span>
                ...
                <div style="display:flex; gap:10px; font-size:9px; border-top:1px dashed #333; margin-top:5px; padding-top:4px;">
                    <span>Tgt: <b>${target}</b></span><span>Capai: <b class="text-gold">${actual}</b></span><span>Sisa: <b class="${sisa>0?'text-red':'text-gold'}">${sisa}</b></span>
                </div>
            </div>
        </div>`;
    });
...
async function openAchieverModal() {
    ...
    const res = globalAchievers.map(x => `
        <div class="achiever-item">
            ...
            <div class="achiever-stats"><span>Target: <b>${x.target}</b></span><span>Capai: <b class="text-gold">${x.actual}</b></span><span>Sisa: <b>0</b></span></div>
        </div>`).join('');
...
function openHistoryModal() {
    const uid = sessionStorage.getItem('userUid');
    const body = document.getElementById('achieverBody');
    document.getElementById('achieverTitle').innerText = "RIWAYAT PERTUMBUHAN SAYA";
    document.getElementById('btnRefreshData').onclick = async () => {
...
function startCountdown(t){if(intervalId)clearInterval(intervalId);intervalId=setInterval(()=>{const d=t-new Date();if(d<=0){clearInterval(intervalId);return}document.getElementById('cdDays').innerText=Math.floor(d/86400000).toString().padStart(2,'0');document.getElementById('cdHours').innerText=Math.floor((d%86400000)/3600000).toString().padStart(2,'0');document.getElementById('cdMin').innerText=Math.floor((d%3600000)/60000).toString().padStart(2,'0');document.getElementById('cdSec').innerText=Math.floor((d%60000)/1000).toString().padStart(2,'0')},1000)}
...
const wordBank={openings:["🔥 SAYA SIAP BERTEMPUR!","🚀 JANGAN LENGAH!","🦁 BANGKITLAH PEMENANG!"],conditions:["Target tidak menunggu kamu siap,","Dunia tidak peduli alasanmu,"],actions:["HANCURKAN rasa malasmu!","berhenti mengeluh dan MULAI BERGERAK!"],closings:["PASTI BISA!","GASPOLL!","TUNTASKAN!"]};
...
function showMotivation(){const m=document.getElementById('motivationModal');if(m){document.getElementById('dailyMotivation').innerText=generateDynamicQuote();m.style.display='flex'}}
