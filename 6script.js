...
async function cleanInvalidVipHistory() {
    ...
    globalHistory.forEach(hist => {
        ...
        if (hist.vip_level > currentRealRank) {
            ...
            invalidIds.push(hist.id); // Asumsi ada kolom id
        }
    });

    if (invalidIds.length > 0) {
        // Hapus dari database (Hanya jika user punya hak delete, jika tidak ini hanya log)
        // db.from('vip_history').delete().in('id', invalidIds).then(res => console.log("Deleted invalid rows:", res));
...
function getRankLevel(uid) {
    ...
    const totalTeam = team.length + 1; // +1 Diri sendiri (sesuai request "Total anggota adalah anda + ...")
    ...
    for (const rule of RANK_RULES) {
        ...
        if (totalTeam >= rule.min) {
            ...
            if (hasStructure || hasGrowth) {
                ...
                // (Kita loop dari 9 ke bawah, jadi ketemu pertama lgsg break)
...
// Cek Growth: 50% berturut-turut pada bulan berjalan (saat tembus) dan bulan berikutnya
function checkGrowthCondition(uid, teamMembers, minMembersReq) {
    ...
    // Karena sortedTeam hanya downline, index ke (req - 2) adalah member trigger (karena +1 self)
...
// Periode 1: Tgl 31 (prev) - 15 (curr)
// Periode 2: Tgl 16 - 30 (curr)
function getPeriodFromDate(date) {
    ...
    if (d >= 16) {
        // Periode 2: 16 - Akhir Bulan (30/31)
        ...
        // Start tgl 31 (atau hari terakhir bulan lalu)
...
function getNextPeriodDates(prevPeriodEndDate) {
    const nextStart = new Date(prevPeriodEndDate.getTime() + 2000); // +2 detik biar aman masuk periode baru
    return getPeriodFromDate(nextStart);
}

function getPreviousPeriodDates(currentPeriodStartDate) {
    const prevEnd = new Date(currentPeriodStartDate.getTime() - 2000); // Mundur 2 detik
    return getPeriodFromDate(prevEnd);
}

// HITUNG TARGET, CAPAI, SISA
function calculatePeriodGrowth(uid, startDate, endDate) {
    const team = getDownlinesRecursive(uid);
    // Base = Member (Self included) join SEBELUM periode dimulai
    // Total Anggota = Anda (1) + Downline join < startDate
    ...
    // Pencapaian = Member join DALAM periode
...
function renderDashboard(){
    ...
    document.getElementById('totalMembers').innerText = downlines.length + 1; // Tampilan Dashboard Total
    ...
    // Label Periode
    const isP2 = now.getDate() >= 16;
    document.getElementById('currentPeriodLabel').innerText = `PERIODE ${isP2?2:1} (${getMonthName(now.getMonth())})`;
    
    startCountdown(currP.end);
    showMotivation();

    // Hitung Statistik Periode Berjalan untuk User Login
    ...
    // Render Grafik Growth (Prev vs Curr)
    ...
    // UPDATE TIM PER LEVEL (Kotak VIP 1-9)
    countVipStats();
}
...
// ============================================================
// LOGIKA TIM PER LEVEL (KOTAK VIP DASHBOARD)
// ============================================================
...
function countVipStats(){
    ...
    const loginUid = sessionStorage.getItem('userUid');

    // Loop semua downline (termasuk user login kalau mau ditampilkan, tapi biasanya downline)
    // Sesuai request: "Daftar anggota yg berhasil mencapai peringkat" (Global atau Tim?)
    // Biasanya Tim Per Level adalah tim sendiri.
    
    myTeamData.forEach(m => {
        // Jangan hitung diri sendiri di kotak dashboard jika itu khusus downline (opsional)
        // Tapi "Tim Per Level" biasanya include user jika dia punya rank.
        ...
        if (r >= 1 && r <= 9) {
            ...
            // Cek Kapan Naik Pangkat (Dari History atau Join Date jika VIP 1 baru)
            // Kita cari di history kapan dia dapet level 'r'
            ...
            // Jika baru < 24 jam, aktifkan notifikasi berkedip
            if ((now - achieveTime) < oneDay) {
                alertStatus[r] = true;
                // Simpan flag di object member untuk list
                m.isNewAlert = true; 
            }
        }
    });
...
// MODAL TIM PER LEVEL (PERIODE BERJALAN)
window.openVipModal = function(level) {
    ...
    // Urutkan: Terbaru mencapai peringkat (berdasarkan history/join)
    const list = vipLists[level].sort((a,b) => {
        ...
        return timeB - timeA; // Descending (Baru ke Lama)
    });
...
// ============================================================
// TOMBOL PERAIH (ACHIEVER) - PERIODE SEBELUMNYA
// ============================================================
...
window.openAchieverModal = function() {
    ...
    const now = new Date();
    const currP = getPeriodFromDate(now);
    const prevP = getPreviousPeriodDates(currP.start); // Periode Target

    const pName = prevP.start.getDate() >= 16 ? `PERIODE 2 (${getMonthName(prevP.start.getMonth())})` : `PERIODE 1 (${getMonthName(prevP.start.getMonth())})`;
    title.innerText = `PERAIH - ${pName}`;
    achieverTxtContent = `REPORT GROWTH 50%\n${pName}\n================\n\n`;

    // 2. Filter Seluruh Anggota Naga Persada yg punya Rank VIP
    // (Bukan cuma tim saya, tapi GLOBAL DATA sesuai request "Daftar seluruh anggota Naga Persada")
    
    let successList = [];
    let failureList = [];

    // Gunakan timeout agar UI tidak freeze saat hitung
    setTimeout(() => {
        globalData.forEach(mem => {
            const rank = getRankLevel(mem.uid);
            
            // Hanya proses jika member adalah VIP (1-9)
            if (rank >= 1) {
                // Hitung Statistik Periode LALU
                ...
                if (gap <= 0) { // Sukses (Sisa 0 atau minus)
...
function renderAchieverList(successList, failureList, body, btnDl) {
    // Urutkan Sukses: Rank Tertinggi, lalu Capaian Tertinggi
    successList.sort((a,b) => {
        if (b.rank !== a.rank) return b.rank - a.rank;
        return b.actual - a.actual;
    });

    // Urutkan Gagal: Rank Tertinggi, lalu Gap Terkecil (yg paling mendekati)
    ...
    if (successList.length > 0) {
        ...
        successList.forEach((a,i) => { 
            ...
            html += `
            <div class="achiever-item" style="border-left:3px solid #00ff00; ${isMe?'background:#1a2a1a;':''}">
                ...
                <div class="achiever-stats">
                    ...
                    <span>Capai: <b class="val-actual" style="color:#00ff00;">${a.actual}</b></span>
                    <span>Sisa: <b style="color:#00ff00;">0</b></span>
                </div>
            </div>`; 
    ...
    if (failureList.length > 0) {
        ...
        failureList.forEach((a,i) => { 
            const isMe = (a.uid === myUid);
            html += `
            <div class="achiever-item" style="border-left:3px solid #ff4444; ${isMe?'background:#2a1a1a;':''}">
                <div class="achiever-top">
                    <span class="v-n">${i+1}. ${a.name} ${isMe?'(ANDA)':''} <small style="color:#888">(VIP ${a.rank})</small></span>
                    <span class="v-u">${a.uid}</span>
                </div>
                <div class="achiever-stats">
                    <span>Target: <b class="val-target">${a.target}</b></span>
                    <span>Capai: <b style="color:#ff4444;">${a.actual}</b></span>
                    <span>Sisa: <b style="color:#ff4444;">${a.gap}</b></span>
                </div>
            </div>`; 
